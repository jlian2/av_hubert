...
class DependencySubmititLauncher(BaseSubmititLauncher):
    ...
    def launch(
        ...
        for j, jp in zip(jobs, job_params):
            ...
            working_dir = os.path.join(sweep_config.hydra.sweep.dir, sweep_config.hydra.sweep.subdir)
            working_dir = re.sub(r"[\'\"\[\]{}]", "", working_dir)
            working_dir = re.sub(",", "_", working_dir)

            subprocess.call(
                [next_script, job_id, task_id, working_dir],
                shell=False,
            )
